...
Copy `.env.example` to `.env` and fill in `DISCORD_TOKEN` / `DISCORD_NEWS_CHANNEL_ID` / `DISCORD_REVIEW_CHANNEL_ID` / `DISCORD_TRAILER_CHANNEL_ID` before running the live bot; `REDDIT_USER_AGENT` needs no API key, just a distinctive string.
...
1. **`sources/reddit.py`** — fetches hot posts via Reddit's public `reddit.com/r/<sub>/hot/.rss` Atom feeds (the official JSON API is gated behind app approval, so this avoids needing a key). Since Atom entries carry no upvote/comment counts, `engagement` is approximated as `(feed position) * SUBREDDIT_WEIGHTS[subreddit]` — the weight exists because feed position alone isn't comparable across subreddits of very different size/activity. Meta-threads (recurring megathreads) are filtered by two layers: an explicit regex list for known phrasings (`_META_THREAD_PATTERNS`), plus a general heuristic — self-post (no `[link]` anchor in the summary) + a thread/discussion word + a date-like token in the title — so new subreddits' own megathread wording doesn't need a hand-written pattern. Also detect whether the feeds is about a trailer or an opencritic thread.
